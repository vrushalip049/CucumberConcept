/*
 * package ReadConfigurationsfromPropertyFile;
 * 
 * import cucumber.api.java.en.Given; import cucumber.api.java.en.When;
 * 
 * public class Steps {
 * 
 ...
 * configFileReader.getDriverPath()); driver = new ChromeDriver();
 * driver.manage().window().maximize();
 * driver.manage().timeouts().implicitlyWait(configFileReader.getImplicitlyWait(
 * ), TimeUnit.SECONDS); pageObjectManager = new PageObjectManager(driver);
 * homePage = pageObjectManager.getHomePage(); homePage.navigateTo_HomePage(); }
 * 
 * @When("^he search for \"([^\"]*)\"$") public void he_search_for(String
 * product) { homePage.perform_Search(product); }
 * 
 ...
 * enter_personal_details_on_checkout_page(){ checkoutPage =
 * pageObjectManager.getCheckoutPage(); checkoutPage.fill_PersonalDetails(); }
 * 
 ...
 * checkoutPage.clickOn_PlaceOrder(); driver.quit(); } }
 */